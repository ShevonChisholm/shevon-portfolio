...
export default function ClientRouteGuard({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      clearStoredSession();
      router.replace("/client/login?error=api_unavailable");
      return;
    }

    if (activeContext && !activeContext.is_client && !activeContext.is_admin) {
      router.replace("/");
    }
  }, [accessToken, activeContext, dispatch, error, hydrated, isLoginRoute, router]);
  ...
  if (
    checkingSession ||
    !accessToken ||
    (activeContext && !activeContext.is_client && !activeContext.is_admin)
  ) {
    return (
      <Box
        sx={{
